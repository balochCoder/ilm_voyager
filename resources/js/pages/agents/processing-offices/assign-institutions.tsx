...
import { ProcessingOffice } from '@/types';

const breadcrumbs = [
  { title: 'Dashboard', href: '/agents/dashboard' },
  { title: 'Processing Offices', href: '/agents/processing-offices' },
  { title: 'Assign Institutions', href: '/agents/processing-offices/assign-institutions' },
];
...
export default function AssignInstitutions({ processingOffice, institutionsByCountry, assignedInstitutionIds }: Props) {
  ...
  useEffect(() => {
    if (shouldSubmit) {
      ...
      post(route('agents:processing-offices:assign-institutions-store', { processingOffice: processingOffice.id }), {
        onError: (errors) => {
  ...
  }, [shouldSubmit, data, post, processingOffice.id]);
      ...
      <div className="flex flex-col gap-4 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 min-w-0">
          <div className="flex-1 min-w-0">
            <Heading title="Assign Institutions" description={`Assign institutions to ${processingOffice.user?.name || processingOffice.name}`} />
          </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-sm">{processingOffice.name}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-muted-foreground">Contact Person:</span>
                <p className="text-sm">{processingOffice.user?.name || 'N/A'}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-muted-foreground">Email:</span>
                <p className="text-sm">{processingOffice.user?.email || 'N/A'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
