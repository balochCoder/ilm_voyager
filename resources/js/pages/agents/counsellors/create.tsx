...
const breadcrumbs = [
  ...
  { title: 'Add Counsellor', href: '/agents/counsellors/create' },
];

export default function CreateCounsellor(props: { branches?: { id: string; name: string }[] }) {
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    branch_id: '',
    as_processing_officer: false as boolean,
  });
  ...
  const handleCheckbox = (name: keyof typeof data, checked: boolean) => {
    if (name === 'download_csv' || name === 'as_processing_officer') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('agents:counsellors:store'));
  };
      ...
      <div className="flex flex-col gap-4 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 min-w-0">
          <div className="flex-1 min-w-0">
            <Heading title="Add Counsellor" description="Create a new counsellor and assign them to a branch." />
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={processing}>
              {processing ? 'Creating...' : 'Create Counsellor'}
            </Button>
          </div>
        </form>
      </div>
