...
import { CounsellorResource } from '@/types';

const breadcrumbs = [
  { title: 'Dashboard', href: '/branch-office/dashboard' },
  { title: 'Counsellors', href: '/branch-office/counsellors' },
  { title: 'Assign Institutions', href: '/branch-office/counsellors/assign-institutions' },
];

interface Props {
  counsellor: CounsellorResource['data'][0];
...
export default function AssignInstitutions({ counsellor, institutionsByCountry, assignedInstitutionIds }: Props) {
  ...
  useEffect(() => {
    if (shouldSubmit) {
      post(route('branches:counsellors:assign-institutions-store', { counsellor: counsellor.id }), {
        onError: () => setShouldSubmit(false),
            ...
            <CardContent>
              ...
              {Object.keys(institutionsByCountry).length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">No active institutions available.</p>
                </div>
              )}
